(function($) {
  ...
  $.kptalks = function(options) {
    ...
    var settings = $.extend({
        ...
        default_text: '你好，我是林智堅', // the words show in popup before loading done
  ...
  function loadData(settings){
    ...
    $.get(settings.api,function(results){
      ...
      var post = posts[Math.floor(Math.random()*(i-1))]
          link = '<a href="'+post.url+'" target="_blank" class="kp_readmore" style="color:'+settings.readmore_color+'">了解更多林智堅的政見</a>';

      var title = (post.title).replace(/【林智堅政見】/g,"");
